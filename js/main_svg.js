...
client.open('GET', 'all_menu.json'); // open的第二個參數放包含餐廳名稱的json
...
function after_get_json() {
	...
	for (var i = 0; i < json_restaurants.menu.length; i++) { // 這行for 中間那坨參考第 17 ~ 20 行會好很多
		var temp_li = document.createElement("li"), // 下三行都是創造 element
			temp_div = document.createElement("div"),
			temp_p = document.createElement("p");

		temp_li.business_id = json_restaurants.menu[i].business_id; // 我額外創了一個叫business_id 的attribute 給li 的DOM 實體，value 給一樣這樣
		temp_p.innerHTML = json_restaurants.menu[i].restaurant_name; // 給值給要顯示的框框
		...
		*/
		...
		temp_array[0].onclick = function() {
				...
				*/
				...
				client.open('GET', "frontend_reviews/restaurant_" + (parseInt(temp_array[1]) + 1) + ".json");
				client.send();
...
(function() {
	...
	client_restaurant.open('GET', 'frontend_restaurant_list.json');
	...
	client_seniment.open('GET', 'frontend_sentiment_words.json');
	client_seniment.onload = function() {
		good_word_array = JSON.parse(client_seniment.responseText).pos_words;
		bad_word_array = JSON.parse(client_seniment.responseText).neg_words;
...
function visualization() {
	...
	(function create_popular_blocks() {
		...
		for (var i = 0; i < 5; i++) { //Tom: 因為我們只要推五道菜，所以i給5
			...
			popular_block.innerHTML = (i + 1) + ". " + popular_array[rest_num].top_dishes[i].name + "";
	...
	(function create_frequency_blocks() {
		...
		for (var i = 0; i < 5; i++) { //Tom: 因為我們只要推五道菜，所以i給5
			...
			// popular_block.style.height = document.getElementsByClassName('block1')[0].offsetHeight * 0.3 + "px";
			//			popular_block.innerHTML = popular_array[i].name;
			most_block.innerHTML = (i + 1) + ". " + popular_array[rest_num].top_dishes[i].name + "";
			...
			p_block_DOM.onmouseover = function() {
				...
				for (var i = 0; i < good_word_array.length; i++) {
					var good_line = makeSVG('path', {
						...
						d: "M" + (((popular_array[rest_num].top_dishes[number].x) + 2) / 2) * w/2 + " " + (((-popular_array[rest_num].top_dishes[number].y) + 2) / 2) * h/2 + "L" + (((good_word_array[i].x) + 2) / 2) * w/2 + " " + (((-good_word_array[i].y) + 2) / 2) * h/2 + "",
						stroke: "#01ADFF",
						"stroke-width": 1
					});
				...
				for (var i = 0; i < bad_word_array.length; i++) {
					var bad_line = makeSVG('path', {
						d: "M" + (((popular_array[rest_num].top_dishes[number].x) + 2) / 2) * w/2 + " " + (((-popular_array[rest_num].top_dishes[number].y) + 2) / 2) * h/2 + "L" + (((bad_word_array[i].x) + 2) / 2) * w/2 + " " + (((-bad_word_array[i].y) + 2) / 2) * h/2 + "",
						stroke: "#E82F2F",
						"stroke-width": 1
					});
	...
	function draw_point() {
		...
		for (var i = 0; i < 5; i++) { //五道菜，所以i給5
			var circle_popular = makeSVG('circle', {
				cx: (((popular_array[rest_num].top_dishes[i].x) + 2) / 2) * w/2,
				cy: (((-popular_array[rest_num].top_dishes[i].y) + 2) / 2) * h/2,
				r: 5,
				stroke: '#141414',
				'stroke-width': 2,
				fill: '#141414'
		...
		for (var i = 0; i < 5; i++) {
			...
			var back_rect = makeSVG('rect', {
				x: (((popular_array[rest_num].top_dishes[i].x) + 2) / 2) * w/2,
				y: (((-popular_array[rest_num].top_dishes[i].y) + 2) / 2) * h/2 - 35,
			...
			text_popular.innerHTML = popular_array[rest_num].top_dishes[i].name;
