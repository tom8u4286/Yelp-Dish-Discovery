window.onload = function(argument) {
	...
	client_rest.open('GET', 'all_menu.json');
	client_rest.onload = function() {

		/* preload every top and most dishes */
		var client_picked = new XMLHttpRequest();
		client_picked.open('GET', 'frontend_restaurant_list.json');
		client_picked.onload = function() {
			picked_json_array = JSON.parse(client_picked.responseText);
		...
		client_word.open('GET', 'frontend_sentiment_words.json');
		client_word.onload = function() {
			good_array = JSON.parse(client_word.responseText).pos_words;
			bad_array = JSON.parse(client_word.responseText).neg_words;
		}
		client_word.send();
	...
	function after_get_rest_json() {
		...
		for (var i = 0; i < rest_json.menu.length; i++) {
			var temp_tr = document.createElement("tr");
			var temp_td = document.createElement("td");
			var temp_div = document.createElement("div");

			temp_div.innerHTML = (i + 1) + ". " + rest_json.menu[i].restaurant_name;
				...
				picked_DOM.onclick = function() {
					...
					for (var i = 0; i < dish_text_array.length; i++) {
						if (dish_text_array[i].dish_name == this.name) {
							for (var j = 0; j < dish_text_array[i].text.length; j++) {
